...
The durable campaign, observability, callback-certification, typed side-effect, pilot-readiness, and evidence-led pilot-operations programme for Days 25–36 is implemented, CI-validated, and browser-verified. Render’s free-service deployment incident was resolved, so the active API is `https://voxflow-voice-agent.onrender.com`; Vercel Production REST and WebSocket origins are both aligned to Render. Render is deliberately request-driven for the owner-mandated free tier, so it may cold-start after idle time and is suitable only for a safe warm-up-and-demo workflow. Release `711305b` adds the Vercel **Prepare Simulator** health-only warm-up flow and guarded migration-managed database startup; GitHub CI run `32494055507` passed API lint, API tests, frontend lint, and frontend build. No real outbound provider call, notification, provider subscription, signing-secret configuration, provider ping, CRM webhook, Sheets write, Gmail fetch, or recording download has been performed during any milestone or verification.

## Verified delivery state

| Area | Verified state |
|---|---|
| Backend quality | `ruff check .` clean; **232 tests passing**. The release adds explicit bootstrap-mode and schema-verification regressions, and isolates test session snapshots from local simulator data. Day 36 evidence controls remain covered. |
...
| Render Free API | Manual Render deployment `dep-da46avrl550s73b3qqg0` checked out `main` commit `711305b`, built successfully, and reported `db.schema_verified dialect=postgresql` before readiness. `GET /api/health` returned `200` with `db_schema_bootstrap_mode="auto"`; a text-only WebSocket probe completed a Hindi agent turn with one read-only tool action. |
| Day 35/Vercel release gates | Vercel Production REST and WebSocket origins both target Render. The live authenticated dashboard remains session-protected; **Prepare Simulator** made the read-only health request, showed `Render Ready` in 0.4s, then the Phone Simulator completed a text-only Hindi agent turn with only the read-only `lookup_supplier` trace. |
...
| Post-Day 36 runtime repairs | `c52fe83` corrects the frontend API origin; `6017b0d` waits for WebSocket `OPEN`; `0047c93` adds reproducible Fly configuration; `2a0151c` selects Groq model `openai/gpt-oss-20b`; `4415ac2` adds Groq reasoning controls; `039c168` degrades safely when hosted TTS is unavailable; `d5cd0ed` serializes simulator-evidence datetimes safely; `94c46d5` restores Fly automatic stop / automatic wake-up; `61bc312` extends simulator WebSocket wake tolerance to 90 seconds; and `711305b` adds health-only simulator preparation plus PostgreSQL schema verification without recurring startup DDL. Render is now active: Vercel `NEXT_PUBLIC_API_URL=https://voxflow-voice-agent.onrender.com` and `NEXT_PUBLIC_WS_URL=wss://voxflow-voice-agent.onrender.com` are deployed together. |
